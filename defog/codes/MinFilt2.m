...
%
%     Changed to suit the edited codes for Van Herk algorithm.
%
...
function Y = MinFilt2(X, varargin)
    % Initialization
    [S, shape] = ParseInputs(varargin{:});

    % filtering
    Y = VanHerk(X, S(1), shape);
    Y = VanHerk(Y, S(2), 'col', shape);
end

function [S, shape] = ParseInputs(varargin)
    shape = 'same';
    flag = [0 0]; % size shape
